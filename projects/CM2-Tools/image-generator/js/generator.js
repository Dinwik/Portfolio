...
async function generate(res=1) {
    retry: while (true) {

        reducedID.innerText = `Generating, please wait.. (attempt ${res})`;
        
        await new Promise(r => setTimeout(r, 25));

        clear();
        
        let original = preview.width*preview.height;
        let reduced = 0;
        
        const width = Math.floor(preview.width / res);
        const height = Math.floor(preview.height / res);
        const qreduced = width*height;
        ...
        reducedID.innerText = `Original: ${original.toLocaleString('en-US')} blocks\nQuality downscale: ${(original-qreduced).toLocaleString('en-US')} blocks\nDummy blocks reduced: ${reduced.toLocaleString('en-US')} blocks\nTotal blocks: ${blocks_.length.toLocaleString('en-US')}`;
        return;
    }
}